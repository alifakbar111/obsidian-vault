...
> ⚠️ **STATUS: SUPERSEDED — kept for historical reference.**
>
> All 18 tasks in this plan have been completed. The backend has since been migrated from Elysia (Bun) to Go (Gin), and a security pass was completed in June 2026. The sections below still describe the **original** Bun/Elysia/Drizzle architecture; for current implementation see the follow-up plans and reviews.
>
> **Superseded by:**
> - [`2026-06-04-migrate-elysia-to-gin.md`](./2026-06-04-migrate-elysia-to-gin.md) — Elysia → Gin + go-jet migration (server rewritten in Go).
> - [`2026-06-04-go-jet-migration.md`](./2026-06-04-go-jet-migration.md) — query builder rollout.
> - [`2026-06-04-unit-testing-plan.md`](./2026-06-04-unit-testing-plan.md) — test strategy.
> - [`../code_review/CODE_REVIEW_2026-06-04.md`](../code_review/CODE_REVIEW_2026-06-04.md) — 39 findings, all closed.
> - [`../code_review/CODE_REVIEW_2026-06-05.md`](../code_review/CODE_REVIEW_2026-06-05.md) — post-fix review.
>
> **Per-task resolution:**
> - Tasks 1–10 (server): completed in Bun/Elysia, then re-implemented in Go/Gin during the 2026-06-04 migration. `server/` no longer exists; current code is in `server-go/`.
> - Tasks 11–16 (web pages): completed in `web/src/pages/`.
> - Task 17 (smoke test): completed; replaced by a 50-parallel rate-limit smoke test in the 2026-06-05 review.
> - Task 17 (Docker): completed; compose file at repo root.
> - Task 17 (a11y audit): completed; all 23 issues addressed.
> - Task 18 (Zod + FormField): completed; schemas in `web/src/lib/schemas/index.ts`.
>
> **Original (Elysia-era) plan retained below.**

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal (original):** Build the backend API server (Elysia) and frontend SPA (React) for Phase 1 — auth, profiles, company verification, candidate search, job postings, and public passport page.

**Architecture (original):** Two separate Bun packages under `skillpass/`: `server/` (Elysia + Drizzle + PostgreSQL) and `web/` (Vite + React 19 + React Router v7 + Tailwind v4 + DaisyUI 5). Server exposes REST API on port 3000, SPA on port 5173 with Vite proxy to backend.

**Current architecture (as of 2026-06-05):** Monorepo with `server-go/` (Go 1.26 + Gin + go-jet + PostgreSQL) on port 1234 and `web/` (Vite + React 19 + React Router v7 + Tailwind v4 + DaisyUI 5) on port 4200. Vite proxies `/api` → `:1234`. No Elysia code remains. Bun is retained only as the workspace script runner (`bun run dev`, `bun run lint`, `bun --cwd web test`). The full file structure of the current project is documented in `AGENTS.md`.

**Tech Stack (original):** Bun, Elysia 1.x, Drizzle ORM, PostgreSQL, React 19, React Router v7, Tailwind CSS v4, DaisyUI 5, `@elysiajs/jwt`, `@elysiajs/swagger`, daisyui

**Tech Stack (current):** Go 1.26, Gin, go-jet (database-first codegen), pgx/v5, PostgreSQL 16, Bun (scripts only), Vite 7, React 19, React Router v7, Tailwind v4 (via `@tailwindcss/vite`), DaisyUI 5, Biome (lint + format), Zod, react-hook-form, `@hookform/resolvers`, `golang.org/x/crypto/bcrypt` + `argon2`, `golang-jwt/jwt/v5`.
...
export const authRoutes = new Elysia({ prefix: '/api/v1/auth' })
  ...
  .post('/register', async ({ body, jwt: j, error }) => {
    ...
    // For company role, use companyName as display name; fallback to 'name'
    const coFields = body as { companyName?: string; businessRegistration?: string; website?: string; address?: string; contact?: string };
    const displayName = role === 'company' ? (coFields.companyName || name) : name;

    const passwordHash = await hashPassword(password);
    const user = await db.insert(schema.users).values({
      email, username, passwordHash, name: displayName, role,
    }).returning();
    ...
    if (role === 'jobseeker') {
      ...
      await db.insert(schema.companies).values({
        ...
        industry: 'Technology',
        verificationDocs: {
          businessRegistration: coFields.businessRegistration ?? '',
          website: coFields.website ?? '',
          address: coFields.address ?? '',
          contact: coFields.contact ?? '',
        },
...
> React Router v7 SPA mode uses `createBrowserRouter` + `<RouterProvider>` (data router pattern), not the JSX `<Routes>/<Route>` + `<BrowserRouter>` pattern.
> Pages are split into **eager** (critical path) and **lazy** (loaded on navigation) for smaller initial bundle. Named exports are wrapped because `React.lazy()` expects `{ default: Component }`.
...
Also create the loading fallback component:

```typescript
// /home/al-ip/learning/skillpass/web/src/components/ui/LoadingFallback.tsx
...
```bash
git -C /home/al-ip/learning/skillpass add web/package.json web/tsconfig.json web/vite.config.ts web/index.html web/src/main.tsx web/src/App.tsx web/src/styles/index.css web/src/components/ui/LoadingFallback.tsx
git -C /home/al-ip/learning/skillpass commit -m "feat: init web frontend with Vite + React + DaisyUI + lazy-loaded routes"
```
...
- [ ] **Write CompanyVerification page (status check + resubmit)**

> **Note (2026-05-30):** Since company verification fields are now collected inline during registration, this page shows verification status and allows resubmission if needed. The initial submission flow has been moved to `Register.tsx`.
...
> The original Bun/Elysia commands below no longer work. Use the current Go workflow.

### Local dev (current, no Docker):
```bash
...
cd skillpass
bun run db:migrate
bun run db:seed   # requires ADMIN_EMAIL + ADMIN_PASSWORD env vars; admin is skipped otherwise

# 3. Start server (Go, port 1234) and web (Vite, port 4200) concurrently
bun run dev
```

`.env` lives in `server-go/`; godotenv loads it and `../.env` automatically. Required vars: `JWT_SECRET`, `DATABASE_URL`. Optional: `PORT` (default 1234), `CORS_ORIGIN` (default `http://localhost:4200`).

### Full Docker setup (current):
```bash
cd skillpass
docker compose up --build
```

Runs `db`, `server-go`, and `web` services. Migrations and seed are not auto-applied — run `bun run db:migrate && bun run db:seed` from the host first, or mount the `migrations/` directory into the server container.

### Original (Elysia-era) run instructions — kept for reference, do not use:
...
## Task 17: Accessibility audit (WCAG 2.1 AA) and remediation

> **Audit date:** 2026-05-23 | **Scope:** All 14 pages + layout components | **Tool:** Manual code review against WCAG 2.1 AA

### Audit summary

**Issues found:** 23 | **Critical:** 6 | **Major:** 10 | **Minor:** 7

### Files to modify

All changes are in `web/src/`. No server changes required.

| File | What to change |
|------|---------------|
| `web/src/styles/index.css` | Add skip-to-content link styles, `:focus-visible` outline overrides, and explicit contrast-safe color tokens |
| `web/src/components/layout/RootLayout.tsx` | Add skip-to-content link at top before `<Navbar />`, ensure `<nav>` semantic element |
| `web/src/components/layout/Navbar.tsx` | Add `aria-expanded`, `aria-haspopup` to dropdown toggle; Escape key handler; fix `<div>` inside `<ul>`; add `aria-hidden` to decorative elements |
| `web/src/components/ui/ThemeToggle.tsx` | Already has `aria-label` — verify and add `aria-hidden` to icon |
| `web/src/components/ui/LoadingFallback.tsx` | Add `role="status"` and `aria-label` to spinner |
| `web/src/pages/Landing.tsx` | Add `aria-hidden="true"` to all `lucide-react` icons |
| `web/src/pages/Login.tsx` | Add `autocomplete` attributes, `aria-describedby` for error, `aria-hidden` on spinner |
| `web/src/pages/Register.tsx` | Convert role toggle to `role="radiogroup"` with `aria-checked`, add `aria-describedby` for field errors, add `autocomplete` attributes, add `aria-hidden` on icons/spinner |
| `web/src/pages/JobseekerProfile.tsx` | Fix invalid `<label>` wrapping `<fieldset>`, add `<h1>`, add `aria-hidden` on icons |
| `web/src/pages/PublicJobs.tsx` | Add `<h1>`, add `aria-label` to industry filter `<select>`, add `aria-hidden` on any icons |
| `web/src/pages/CompanySearch.tsx` | Add `role="search"` to search form, add `<h1>`, add `aria-label` to inputs |
| `web/src/pages/JobDetail.tsx` | Add `<h1>`, add `aria-hidden` on icons |
| `web/src/pages/JobseekerPassport.tsx` | Add `<h1>`, add `aria-hidden` on icons |
| `web/src/pages/CompanyProfile.tsx` | Add `<h1>`, add `aria-hidden` on icons |
| `web/src/pages/CompanyVerification.tsx` | Add `<h1>`, add `aria-hidden` on icons |
| `web/src/pages/CompanyJobs.tsx` | Add `<h1>`, add `aria-hidden` on icons |
| `web/src/pages/AdminVerifications.tsx` | Add `<h1>`, add `aria-hidden` on icons |
| `web/src/index.html` | Add `<meta name="description">` and ensure `<html lang="en">` is present |

### Critical fixes

1. **🔴 Skip-to-content link** — Add at top of RootLayout. Affects all keyboard/screen reader users, blocks access to main content.
2. **🔴 Navbar dropdown keyboard handling** — Add `aria-expanded`, `aria-haspopup`, Escape key handler, and focus management. Affects all authenticated users.
3. **🔴 Register role toggle as radio group** — Convert to semantic `role="radiogroup"` with `role="radio"` + `aria-checked`. Affects all new users registering.
4. **🔴 JobseekerProfile `<label>` containing `<fieldset>`** — Fix invalid HTML nesting. Affects jobseeker profile editing.
5. **🔴 `opacity-60` / `opacity-70` text contrast** — Replace opacity with explicit colors meeting 4.5:1 ratio. Affects landing page, feature cards, footer.
6. **🔴 `<div>` inside `<ul>` in Navbar dropdown** — The `<div className="divider">` is a direct child of `<ul>`, which is invalid HTML. Move it out of the `<ul>` or wrap in `<li>`.

### Major fixes

7. **🟡 Add `aria-hidden="true"` to all decorative icons** — All `lucide-react` icons (`Briefcase`, `Search`, `Award`, `Plus`, `Pencil`, `Trash2`, `Sun`, `Moon`) across all pages must have `aria-hidden={true}`.
8. **🟡 Form error `aria-describedby` associations** — Links error messages to inputs via `aria-describedby` referencing the error element's `id`. Affects all forms (Login, Register, Profile, etc.).
9. **🟡 Loading spinner accessible name** — Add `role="status"` and `aria-label="Loading..."` to all loading spinners.
10. **🟡 Semantic HTML for navbar** — Replace `<div className="navbar">` with `<nav aria-label="Main navigation">`. Affects all pages.
11. **🟡 Focus visible indicators** — Ensure `:focus-visible` outlines are globally visible in `index.css`.
12. **🟡 Missing `<h1>` on most pages** — Each page should have a descriptive `<h1>` heading. Currently only Login/Register have `<h2>`.
13. **🟡 Add `aria-live="polite"` or `role="alert"` to LoadingFallback** — For Suspense loading states, screen readers need to be notified.
14. **🟡 Add `role="search"` to search forms** — CompanySearch search form needs `role="search"` landmark.

### Minor fixes

15. **🟢 Add `autocomplete` attributes** — Login: `autocomplete="email"` and `autocomplete="current-password"`. Register: `autocomplete="name"`, `autocomplete="username"`, `autocomplete="email"`, `autocomplete="new-password"`.
16. **🟢 Verify badge contrast** — DaisyUI badge/neutral colors may dip below 3:1 for non-text contrast.
17. **🟢 Consider moving ThemeToggle before avatar dropdown in DOM order** — For logical focus order.

### Implementation order

1. `web/src/styles/index.css` — Base styles (skip link, focus indicators, contrast colors)
2. `web/src/index.html` — Meta description
3. `web/src/components/layout/RootLayout.tsx` — Skip link + semantic `<nav>`
4. `web/src/components/layout/Navbar.tsx` — Dropdown a11y + semantic HTML
5. `web/src/components/ui/LoadingFallback.tsx` — Spinner a11y
6. `web/src/pages/Landing.tsx` — Icon `aria-hidden` + contrast
7. `web/src/pages/Login.tsx` — Form a11y
8. `web/src/pages/Register.tsx` — Role toggle radio group + form a11y
9. `web/src/pages/JobseekerProfile.tsx` — `<label>`/`<fieldset>` fix + `<h1>` + icons
10. `web/src/pages/JobseekerPassport.tsx` — `<h1>` + icons
11. `web/src/pages/PublicPassport.tsx` — `<h1>` + icons
12. `web/src/pages/CompanyProfile.tsx` — `<h1>` + icons
13. `web/src/pages/CompanyVerification.tsx` — `<h1>` + icons
14. `web/src/pages/CompanySearch.tsx` — `<h1>` + `role="search"` + icons
15. `web/src/pages/CompanyJobs.tsx` — `<h1>` + icons
16. `web/src/pages/PublicJobs.tsx` — `<h1>` + `<label>` for filter + icons
17. `web/src/pages/JobDetail.tsx` — `<h1>` + icons

---

### Task 18: Add Zod form validation + reusable FormField component

> **Goal:** Replace all inline validation (raw `useState`/`FormEvent` + HTML `required`) and Register's `react-hook-form` inline rules with Zod schemas + `zodResolver`. Build a reusable `<FormField>` wrapper to eliminate boilerplate across all 6 form pages.

**Files:**
- Modify: `web/package.json` — add `zod` + `@hookform/resolvers`
- Create: `web/src/lib/schemas/index.ts` — all Zod schemas + inferred types
- Create: `web/src/components/ui/FormField.tsx` — reusable input/select/textarea wrapper with error display
- Modify: `web/src/pages/Login.tsx`
- Modify: `web/src/pages/Register.tsx`
- Modify: `web/src/pages/JobseekerProfile.tsx`
- Modify: `web/src/pages/CompanyProfile.tsx`
- Modify: `web/src/pages/CompanyJobs.tsx`
- Modify: `web/src/pages/CompanyVerification.tsx`

**No server changes.** Server stays on Elysia's `t.*` (TypeBox) validation — no Zod on the backend.

- [ ] **Install dependencies**
```bash
bun --cwd web add zod @hookform/resolvers
```

- [ ] **Create `web/src/lib/schemas/index.ts`**

One schema per form domain, plus inferred types:
...
Single component that renders `label.form-control` with appropriate DaisyUI input/select/textarea + error message. Avoids duplicating the label-input-error pattern across every page.

Props:
- `label: string` — visible label text
- `name: string` — field name (used for error lookup)
- `type?: 'input' | 'select' | 'textarea'` — defaults to `'input'`
- `options?: {value: string, label: string}[]` — for `type: 'select'`
- `placeholder?: string`
- `disabled?: boolean`
- `register: UseFormRegister<...>` — from react-hook-form
- `error?: FieldError` — from `formState.errors[name]`
- Plus spread `...rest` for `type="number"`, `autoComplete`, etc.
...
<label className="form-control w-full">
  ...
  {error && <span className="text-error text-xs mt-1" role="alert">{error.message}</span>}
</label>
```

- [ ] **Convert `Login.tsx`**

Replace `useState<string>` × 2 + `FormEvent` handler with:
```tsx
...
Use `<FormField>` for email and password. Keep `serverError` display. Use `aria-describedby` on server error.

- [ ] **Convert `Register.tsx`**

Replace inline rules in `register('name', { required: ... })` with `zodResolver(registerSchema)`. Remove all `required`/`minLength`/`pattern` from the `register()` calls — they're in the schema now. Use `<FormField>` for inputs and `<div className="flex gap-2">` with role toggle buttons (same pattern, stays manual).

- [ ] **Convert `JobseekerProfile.tsx`**

Split into two `useForm` instances:
1. **Profile form** — `useForm<ProfileForm>({ resolver: zodResolver(profileSchema) })`. Replace `saveProfile` `FormEvent` with `handleSubmit`. Use `<FormField type="textarea">` for about, `<FormField type="input">` for headline, `<FormField type="input" {...register('yearsOfExperience')}>` with `type="number"`.
2. **Experience form** — `useForm<ExperienceForm>({ resolver: zodResolver(experienceSchema) })`. Replace `addExperience` `FormEvent` with `handleSubmit`. Use `<FormField type="select">` for type, `<FormField>` for text inputs, `<FormField type="textarea">` for description. Keep skills parsing (`split(',').map(...)`) in the submit handler.

- [ ] **Convert `CompanyProfile.tsx`**

Replace `useState` + `FormEvent` with `useForm<CompanyProfileForm>({ resolver: zodResolver(companyProfileSchema) })`. Use `<FormField>` for all fields, `<FormField type="select">` for industry dropdown with `options` from API.

- [ ] **Convert `CompanyJobs.tsx`**

Replace `useState` + `FormEvent` with `useForm<JobForm>({ resolver: zodResolver(jobSchema) })`. Use `<FormField>` for all fields. Keep tags/skills parsing in the submit handler.

- [ ] **Convert `CompanyVerification.tsx`**

Replace `useState` + `FormEvent` with `useForm<VerificationForm>({ resolver: zodResolver(verificationSchema) })`. Use `<FormField>` for all fields.

- [ ] **Verify typecheck + lint**
```bash
bun --cwd web typecheck && bun run lint
```
